function [date_result] = dataset_test(file_path_im,file_path_manual,im_postfix,ma_postfix)
...
if img_num > 0 % 有满足条件的图像
        for i = 1:img_num % 逐一读取图像
            ...
            %图像处理过程 
            date_result(i).image = image_name;
            date_result(i).manual = manual_name;
            
            [Se, Sp, ~, ~, Acc, Dice] = retinal_vessel_seg(image,manual);
            date_result(i).Accuracy = Acc;
            date_result(i).Sensitivity = Se;
            date_result(i).Specificity = Sp;
            date_result(i).Dice = Dice;
        end
end
